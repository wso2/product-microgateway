...
func startBrokerConsumer(connectionString string, sub Subscription, reconnectInterval time.Duration) {
	...
	for {
		...
		func() {
			defer logger.LoggerMsg.Errorf("ASB consumer has stopped for subscription: %s, topic: %s", subName, topic)
			ctx, cancel := context.WithCancel(parentContext)
			defer cancel()

			// keep receiving messages from asb
			for {
				logger.LoggerMsg.Debugf("Continue processing messages from ASB for subscription: %s, topic: %s", subName, topic)
				...
				logger.LoggerMsg.Debugf("Processed %v messages from ASB for subscription: %s, topic: %s", len(messages), subName, topic)
			}
		}()
	}
}
