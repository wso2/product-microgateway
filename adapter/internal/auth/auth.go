...
func GenerateToken(username string) (accessToken string, err error) {
	...
	if err != nil {
		loggers.LoggerAuth.ErrorC(logging.ErrorDetails{
			Message:   fmt.Sprintf("failed to generate signed payload: %s", err),
...
func ValidateToken(accessToken string, resourceScopes []string, conf *config.Config) (
	...
	if err != nil {
		loggers.LoggerAuth.ErrorC(logging.ErrorDetails{
			Message:   fmt.Sprintf("Failed to retrive private key: %s", err),
	...
	if err != nil {
		loggers.LoggerAPI.ErrorC(logging.ErrorDetails{
			Message:   fmt.Sprintf("Failed to parse JWT token: %s", err),
