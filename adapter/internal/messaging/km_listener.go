...
func handleKMConfiguration(deliveries <-chan amqp.Delivery, done chan error) {
	...
	for d := range deliveries {
		...
		if unmarshalErr != nil {
			logger.LoggerMsg.Errorf("Error occured while unmarshalling key manager event data %v", unmarshalErr)
			return
		}
		...
		if err != nil {
			...
			logger.LoggerMsg.Errorf("Error occured while decoding key manager configuration data %v", err)
			return
		}
