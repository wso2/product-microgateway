...
public class FaultResponsesTestCase extends BaseTestCase {
    protected String jwtTokenProd;

    @BeforeClass
    public void start() throws Exception {
        String project = "faultResponsesProject";
        // Define application info
        ApplicationDTO application = new ApplicationDTO();
        application.setName("faultResponsesApp");
        ...
        super.init(project, new String[]{"common_api.yaml"}, null, null,
    ...
    public void testMethodNotAllowed() throws Exception {
        Map<String, String> headers = new HashMap<>();
        org.wso2.micro.gateway.tests.util.HttpResponse response = HttpClientRequest
                .doPost(getServiceURLHttp("petstore/v1/pet/findByStatus"), "{}", headers);
        Assert.assertNotNull(response);
    ...
    public void testMethodNotFound() throws Exception {
        Map<String, String> headers = new HashMap<>();
        org.wso2.micro.gateway.tests.util.HttpResponse response = HttpClientRequest
                .doGet(getServiceURLHttp("petstore/v1/abc"), headers);
        Assert.assertNotNull(response);
        Assert.assertEquals(response.getData(), ResponseConstants.NOT_FOUND_RESPONSE);
        Assert.assertEquals(response.getResponseCode(), 404, "Response code mismatched");
    }

    @Test(description = "Test the normal API invocation when fault responses are enabled")
    public void testAPINotFound() throws Exception {
        Map<String, String> headers = new HashMap<>();
        org.wso2.micro.gateway.tests.util.HttpResponse response = HttpClientRequest
                .doGet(getServiceURLHttp("petstore/abc"), headers);
        Assert.assertNotNull(response);
        Assert.assertEquals(response.getData(), ResponseConstants.NOT_FOUND_RESPONSE);
        Assert.assertEquals(response.getResponseCode(), 404, "Response code mismatched");
    }
