 ...
 */
public class SubscriptionPolicyDiscoveryClient implements Runnable, DiscoveryClient {
    ...
    public static SubscriptionPolicyDiscoveryClient getInstance() {
        ...
        HealthService.registerDiscoveryClient("SubscriptionPolicyDiscoveryClient", instance);
        return instance;
    }
     ...
     */
    private void ack() {
        logger.debug("Applied Discovery Response for type url: " + Constants.SUBSCRIPTION_POLICY_LIST_TYPE_URL);
        DiscoveryRequest req = DiscoveryRequest.newBuilder()
                .setNode(node)
