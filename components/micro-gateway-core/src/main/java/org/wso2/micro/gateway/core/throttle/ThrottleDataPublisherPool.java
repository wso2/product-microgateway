...
/**
 * This class implemented to hold throttle data processing agent pool. Reason for implement this is to
...
public class ThrottleDataPublisherPool {
    ...
    private static final Logger log = LoggerFactory.getLogger(ThrottleDataPublisherPool.class);
    ...
    private ThrottleDataPublisherPool() {
        ...
        // are created to support the new load. Hence this following data structure places no limit on the number of "
        ...
        clientPool = new StackObjectPool(new BasePooledObjectFactory(), throttleConfigHolder.getProcessPoolMaxIdle(),
                throttleConfigHolder.getProcessPoolInitIdleCapacity());
    }
    ...
    public DataProcessAgent get() throws Exception {
        return (DataProcessAgent) clientPool.borrowObject();
    }

    public void release(DataProcessAgent client) throws Exception {
    ...
    static class BasePooledObjectFactory extends BasePoolableObjectFactory {
        @Override
        public Object makeObject() throws Exception {
            if (log.isDebugEnabled()) {
                log.debug("Initializing new ThrottleDataReceiver instance");
            }
            return new DataProcessAgent();
        }
    }
