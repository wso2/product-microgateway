...
public type Analytics4xRequestFilterWrapper object {
    Analytics4xRequestFilter analytics4xRequestFilter = new;
    ...
    public function filterRequest(http:Caller caller, http:Request request, http:FilterContext context) returns boolean {
        ...
        boolean result = self.analytics4xRequestFilter.filterRequest(caller, request, context);
    ...
    public function filterResponse(http:Response response, http:FilterContext context) returns boolean {
        ...
        if (gaugeTags is ()) {
            gaugeTags = gaugeTagDetailsFromContext(context, FILTER_ANALYTICS);
        }
        ...
        int startingTime = getCurrentTime();
        boolean result = self.analytics4xRequestFilter.filterResponse(response, context);
        float | () latency = setGaugeDuration(startingTime);
