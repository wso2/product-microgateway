...
# + return - string []
public function etcdAllRevokedTokenLookup() returns @tainted map<string> {
    http:Request req = new;
        ...
        if (msg is json) {
            map<json>|error message = map<json>.constructFrom(msg);
              if (message is map<json>) {
                    printDebug(KEY_TOKEN_REVOCATION_ETCD_UTIL, "etcd responded with a payload");
                    map<json>|error node = map<json>.constructFrom(message["node"]);
                        if (node is map<json>) {
                            json[] nodes = <json[]>node["nodes"];
                            ...
                            printDebug(KEY_TOKEN_REVOCATION_ETCD_UTIL, "Error in retrieving json map");
                            printError(KEY_TOKEN_REVOCATION_ETCD_UTIL, node.reason());
                        }
                } else {
                    printDebug(KEY_TOKEN_REVOCATION_ETCD_UTIL, "Error in retrieving json map");
                    printError(KEY_TOKEN_REVOCATION_ETCD_UTIL, message.reason());
                }
        } else {
            printDebug(KEY_TOKEN_REVOCATION_ETCD_UTIL, "Error in retrieving json object");
            printError(KEY_TOKEN_REVOCATION_ETCD_UTIL, msg.reason());
