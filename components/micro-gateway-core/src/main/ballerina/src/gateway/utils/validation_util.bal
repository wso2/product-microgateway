...
function read(string path) returns @tainted json {
...
public function valueValidator(string key, json value, json field) returns (error?[]) {
    ...
    error?[] errors = [];
    json|error minVal = field.minimum;
    json|error maxVal = field.maximum;
    int intValue = 0;
    if (field.TYPE.toString() == STRING && field.format == null) {
        ...
        if (min > 0 || max > 0)  {
            json|error minLength = field.minLength;
            json|error maxLength = field.maxLength;
            if (minLength is json && maxLength is json) {
                processError(errors, validateMinMaxLength(key, value, minLength, maxLength));
            }
        }
    }
    //if a pattern is defined for the string, validate the pattern
    if (field.pattern != null) {
        json|error pattern = field.pattern;
        if (pattern is json) {
            processError(errors, validatePattern(key, value, pattern));
        }
    }
    ...
    if (field.TYPE.toString() == INTEGER || field.TYPE.toString() == NUMBER) {
        if (typeOf(value) == INTEGER) {
            ...
            if ((minVal != null || maxVal != null) && (minVal is json && maxVal is json)) {
                    processError(errors, validateMinMaxValue(key, intValue, minVal, maxVal, false, false));
                }
            }
            if (field.exclusiveMinimum != null || field.exclusiveMaximum != null) {
                ...
                if ((minVal != null || maxVal != null) && (minVal is json && maxVal is json)) {
                    processError(errors, validateMinMaxValue(key, intValue, minVal, maxVal,
            ...
            if ((minVal != null || maxVal != null) && (minVal is json && maxVal is json)) {
                processError(errors, validateMinMaxValue(key, floatValue, minVal, maxVal, false, false));
            }
            if (field.exclusiveMinimum != null || field.exclusiveMaximum != null) {
                ...
                //validate minimum/maximum with exclusiveMinimum/exclusiveMaximum properties
                if ((minVal != null || maxVal != null) && (minVal is json && maxVal is json)) {
                    processError(errors, validateMinMaxValue(key, floatValue, minVal, maxVal, exclusiveMin, exclusiveMax));
                }
            }
        }
    //if field contains a enum validate it
    if (field.enum != null) {
        json|error fieldEnum = field.enum;
        if (fieldEnum is json) {
            processError(errors, validateEnums(key, value, fieldEnum));
        }
    }
    return errors;
   }
...
public function validateMinMaxLength(string name, json value, json minLength, json maxLength) returns (boolean|error) {
    ...
    //if there is both minLength and maxLength properties, validate them
...
public function validateMaxLength(string name, string value, int maxLength) returns (boolean|error) {
    ...
    if (maxLength > 0) {
        if (value.length() > maxLength) {
            err = error(name + " must be no more than " + maxLength.toString() + " characters long");
            return err;
        }
    }
    return true;
...
public function validateMinLength(string name, string value, int minLength) returns (boolean|error) {
    ...
    if (minLength > 0) {
        if (value.length() < minLength) {
            ...
            err = error(name + " must be at least " + minLength.toString() + " characters long");
            return err;
        }
    }
    return true;
...
public function validatePattern(string name, json value, json pattern) returns (boolean|error) {
    ...
    if (matches(stringValue, newRegex)) {
        return true;
    } else {
...
public function validateMinMaxValue(string name, int|float value, json minValue, json maxValue, boolean exclusiveMin
    ...
    if (value is int) {
        if (!exclusiveMin && !exclusiveMax && (value < min || value > max)) {
            err = error(name + " must be at least " + min.toString() + " and no more than " +
                max.toString());
            return err;
        } else if (exclusiveMin && exclusiveMax && (value <= min || value >= max)){
            err = error(name + " must be greater than " + min.toString() + " and less than " +
                max.toString());
            return err;
        } else if (exclusiveMin && (value <= min || value > max)){
            err = error(name + " must be greater than " + min.toString() + " and no more than "
                + max.toString());
            return err;
        } else if (exclusiveMax && (value >= max || value < min)){
            err = error(name + " must be at least " + min.toString() + " and less than " + max.toString());
            return err;
        }
    }
    else {
        if (!exclusiveMin && !exclusiveMax && (value < min || value > max)) {
            err = error(name + " must be at least " + min.toString() + " and no more than " + max.toString());
            return err;
        } else if (exclusiveMin && exclusiveMax && (value <= min || value >= max)){
            err = error(name + " must be greater than " + min.toString() + " and less than " +
                max.toString());
            ...
            err = error(name + " must be greater than " + min.toString() +
                " and no more than " + max.toString());
            return err;
        } else if (exclusiveMax && (value >= max || value < min)){
            err = error(name + " must be at least " + min.toString() + " and less than " + max.toString());
            return err;
...
public function validateMinValue(string name, int|float value, int minValue, boolean exclusiveMin)
                    returns (boolean|error) {
    error err;
    //value can be of type integer or float
    if (value is int) {
        if (!exclusiveMin && value < minValue) {//if there is no exclusiveMinimum property
            err = error(name + " must be at least " + minValue.toString());
            return err;
        } else if (exclusiveMin && value <= minValue) {//if there is exclusiveMinimum property
            err = error(name + " must be greater than " + minValue.toString());
            return err;
        }
    }
    else {
...
public function validateMaxValue(string name, int|float value, int maxValue, boolean exclusiveMax)
                    returns (boolean|error) {
    error err;
    //value can be of type integer or float
    if (value is int) {
        if (!exclusiveMax && value > maxValue) {//if there is no exclusiveMinimum property
            err = error(name + " must not be more than " + maxValue.toString());
            return err;
        } else if (exclusiveMax && value >= maxValue) {//if there is exclusiveMinimum property
            err = error(name + " must be less than " + maxValue.toString());
            return err;
        }
    }
    else {
...
public function getMergedModel(json model, json models) returns (json) {
    ...
    if (model.allOf != null && models is map<json>) {
        json merged = ();
        foreach var [k, v] in models.entries() {
            if (v.REFERENCE is ()) {
...
public function getDiscriminatedModel(json target, json model, json models) returns (json) {
    string subModelName = "";
    json newModel = model;
    if (target.model.discriminator.propertyName != null) {
...
public function getReferencedModel(json model, json models) returns (json) {
    json outModel = model;
    json|error modelProperties = model.properties;
    json modRef = {};
    if (modelProperties is json) {
    if (modelProperties != null && modelProperties is map<json>) {
        var keys = modelProperties.keys();
        foreach var key in keys {
            var item = modelProperties[key];
            //if there is a reference get the referenced model
            if (item != null && item.REFERENCE != null) {
                string reference = item.REFERENCE.toString();
                string replacement = replaceModelPrefix(reference);
                var modelReference = models.replacement;
                if (modelReference != null) {
                    //if there is allOf property in the referenced model get the merged model
                    if (modelReference is json) {
                        modRef = <json>modelReference.allOf;
                        ...
                        modelProperties[key] = merge(item, modelReference);
                    }
                }
            }
        }
...
public function merge(json targetModel, json sourceModel) returns (json) {
    json|error targetKey = {};
    if (sourceModel is map<json> && targetModel is map<json>) {
      foreach var key in sourceModel.keys() {
        json|error sourceProperty = sourceModel.key;
        if (sourceProperty is json) {
           if (targetModel[key] == null) {//if there are no such property in the targetModel add it
            ...
            targetKey = targetModel[key];
            if (targetKey is map<json>) {
                if (sourceProperty is map<json>) {
                    if (sourceProperty.keys().length() > 0) {
                        foreach var proprtyKey in sourceProperty.keys() {
                            if (targetKey[proprtyKey] is ()) {
                                targetKey[proprtyKey] = targetKey[proprtyKey];
                            }
                        }
                    }
                }
...
public function validateType(string name, json value, json field, json models) returns @tainted (Result) {
    Result result = new();
    ...
    if (field.TYPE != ()) {
        ...
        if (expectedType.length() > 0) {//if the field doesn't have a type it may have a reference to another model
            if (models != null && field.REFERENCE != null) {
                ...
                json|error fieldRef = models.fieldReference;
                if (fieldRef is json) {
                    return validate(name, value, fieldRef, models);
                } else {
                    //todo
                }
            }
        } else {
            expectedType = toLowerCase(expectedType);
        }
...
public function validateObject(string name, json value, json field, json models) returns @tainted(Result) {
    Result result = new();
...
//validating an array
public function validateArray(string name, json value, json field, json models) returns @tainted(Result) {
    json[] mapValue = <json[]>value;
    Result result = new();
    json|error items = field.ITEMS;
    ...
    if (items != null && items.TYPE != null) {
        ...
        var fieldType = items.TYPE;
        error?[] firstErrorArray = [];
        //loop each item in the array and validate them
        foreach var v in mapValue {
            json newVal = v;
            var valueErrors = validateValue(name + countItems.toString(), items, newVal, models);
        ...
        arrayType = items.REFERENCE.toString();
    }
}
    ...
    if (minItems > 0 && countItems < minItems) {
        error err = error("Array requires at least " + minItems.toString() + " item(s) and has " + countItems.toString() +
            " item(s).");
    ...
    if (maxItems > 0 && countItems > maxItems) {
        error err = error("Array requires no more than " + maxItems.toString() + " item(s) and has " + countItems.toString() +
            " item(s).");
        thirdErrorArray[i] = err;
        i = i + 1;
        return createReturnObject(thirdErrorArray, "Array of " + arrayType + " (" + name + ")");
...
public function validateSpecification(string name, json target, json model, json models) returns (error?[]) {
    map<json> mapValue = <map<json>>target;
    ...
    if (properties == null) {
        ...
        if (<string>typeOf(target) != ARRAY) {
            ...
            if (model.discriminator != null && errorArray.length() >= 1) {
                ...
                //that property
                error? errorProperty = errorArray[0];
                if (errorProperty is error) {
                    if (errorArray.length() == 1 && errorProperty.reason() == "Target property " + model.discriminator.
                        propertyName.toString() + " is not in the model") {
                        // remove discriminator if it is the only error.
                        errorArray = [];
                    } else {
                        error?[] tempErrors = errorArray;
                        if (tempErrors is error[]) {
                        errorArray = [];
            ...
            foreach var key in properties.keys() {
                ...
                if (value != null && value is json && field is json) {
...
public function validate(string name, json target, json swaggerModel, json swaggerModels) returns @tainted (Result) {
    error err = error("");
    if (target == null) {
    ...
    json|error requiredModel = model.required;
    json|error modelProperties = model.properties;
    if (model.required != null && requiredModel is json && modelProperties is json) {
        error?[] requireFieldErrors = [];
        requireFieldErrors = validateRequiredFields(target, requiredModel, modelProperties);
...
public function validateRequiredFields(json target, json fields, json modelFields) returns (error?[]) {
    ...
    if (target is json[]) {
        mapValue = <map<json>>target[1];
    } else {
        mapValue = <map<json>>target;
    }
    ...
    if (fields is map<json>) {
        foreach var[k, v] in fields.entries() {
            var property = fields[k].toString();
            if (<string>typeOf(target) == ARRAY) {
                foreach var [key, val] in mapValue.entries() {
                    if (val.property == null || modelFields.property == null) {
...
//validating properties in the JSON object against the properties of the models defined in the schema.
public function validateProperties(json target, json model, json models) returns (error?[]) {
    error?[] errorArray = [];
    int i = 0;
    if (target != () && target is map<json>) {
        // var targetKeys = target.keys();
        string[] targetKeys = target.keys();
