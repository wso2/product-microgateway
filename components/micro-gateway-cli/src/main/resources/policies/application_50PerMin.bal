...
function initApplication50PerMinPolicy() {
    ...
    forever {
        ...
        select intermediateStream.throttleKey, getThrottleValue50(intermediateStream.eventCount) as isThrottled, intermediateStream.stopOnQuota, intermediateStream.expiryTimeStamp
        group by eligibilityStream.throttleKey
...
function getThrottleValue50(int eventCount) returns boolean{
    if(eventCount>=50){
        return true;
    }else{
        return false;
