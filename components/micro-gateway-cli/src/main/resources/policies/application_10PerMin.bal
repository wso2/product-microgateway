...
function initApplication10PerMinPolicy() {
    ...
    forever {
        ...
        select gateway:requestStream.messageID, (gateway:requestStream.appTier == "10PerMin") as isEligible, gateway:requestStream.appKey as throttleKey
        ...
        select intermediateStream.throttleKey, getThrottleValue10(intermediateStream.eventCount) as isThrottled, intermediateStream.stopOnQuota, intermediateStream.expiryTimeStamp
        group by eligibilityStream.throttleKey
...
function getThrottleValue10(int eventCount) returns boolean{
    if(eventCount>=10){
        return true;
    }else{
        return false;
