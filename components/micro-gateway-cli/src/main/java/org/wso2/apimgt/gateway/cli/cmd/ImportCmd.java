...
public class ImportCmd implements GatewayLauncherCmd  {
    ...
    public void execute() {
        ...
        if (isOpenApi) {
            ...
            validateAPIGetRequestParams(apiName, version);
            //Setup username
            ...
            try {
                ...
                outStream.println("Importing the API is successful.");
    ...
    /**
     * Validates  API name and version parameters in for below conditions.
     * @param apiName API name
     * @param version API version
     */
    private void validateAPIGetRequestParams(String apiName, String version) {
        if ((StringUtils.isEmpty(apiName) || StringUtils.isEmpty(version)) ||
                (StringUtils.isNotEmpty(apiName) || StringUtils.isNotEmpty(version)) ||
            ...
            throw GatewayCmdUtils.createUsageException(
                    "Either label (-l <label>) or API name (-a <api-name>) with version (-v <version>) "
                            + "should be provided."
                            + "\n\tmicro-gw import pizzashack-project -a Pizzashack -v 1.0.0");
        }
    }
