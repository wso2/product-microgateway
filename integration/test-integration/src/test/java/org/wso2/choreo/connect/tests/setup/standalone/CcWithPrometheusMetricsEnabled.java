...
public class CcWithPrometheusMetricsEnabled {
    CcInstance ccInstance;

    @BeforeTest(description = "initialise the setup")
    void start() throws Exception {
        ccInstance = new CcInstance.Builder().withNewDockerCompose("cc-prometheus-metrics.yaml")
                .withNewConfig("prometheus-metrics-enabled.toml")
                .withVolumeMountDir("metricConfigs")
                .build();
        ccInstance.start();
