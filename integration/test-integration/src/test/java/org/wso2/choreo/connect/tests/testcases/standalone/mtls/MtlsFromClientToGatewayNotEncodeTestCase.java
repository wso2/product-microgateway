...
public class MtlsFromClientToGatewayNotEncodeTestCase {
    ...
    @Test(description = "Invoke with Mandatory MTLS and Mandatory OAuth2 & ApiKey enabled API with client cert in" +
            " header")
    public void invokeMTLSMandatoryOauth2ApiKeyMandatoryAPIHeaderSuccessTest() throws Exception {
        ...
        headers.put(TestConstant.CLIENT_CERT_HEADER, TestConstant.CLIENT_CERT);
    ...
    @Test(description = "Invoke with Mandatory MTLS and Mandatory OAuth2 & ApiKey enabled API with invalid client " +
            "cert in header")
    public void invokeMTLSMandatoryOauth2ApiKeyMandatoryAPIHeaderFailTest2() throws Exception {
        ...
        headers.put("apiKey", TestConstant.MUTUAL_SSL_API_KEY);
        headers.put(HttpHeaderNames.AUTHORIZATION.toString(), "Bearer " + jwtTokenProd);
        headers.put(TestConstant.CLIENT_CERT_HEADER, TestConstant.INVALID_CLIENT_CERT);
        HttpResponse response = HttpsClientRequest.doGet(Utils.getServiceURLHttps(
                "/mtls/1.0.0/pet/findByStatus"), headers);
    ...
    @Test(description = "Invoke with Optional MTLS and Mandatory ApiKey enabled API with client cert in header")
    ...
    @Test(description = "Invoke with Optional MTLS and Mandatory ApiKey enabled API with invalid client cert in header")
    ...
    @Test(description = "Invoke with Mandatory MTLS and Optional OAuth2 enabled API with client cert in header")
    ...
    @Test(description = "Invoke with Mandatory MTLS and Optional OAuth2 enabled API with invalid client cert in header")
